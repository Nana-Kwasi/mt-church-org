...
const [showOtpDisableModal, setShowOtpDisableModal] = useState(false);
const [otpDisableForm, setOtpDisableForm] = useState({
  name: '',
  email: '',
  reason: ''
});

const [isSubmittingOtpDisable, setIsSubmittingOtpDisable] = useState(false);
const [otpDisableMessage, setOtpDisableMessage] = useState('');
  ...
  const checkOtpDisableStatus = async (userEmail) => {
  const db = getFirestore();
  try {
    const otpDisableRef = collection(db, 'OTPDisable');
    const q = query(otpDisableRef, where('email', '==', userEmail.toLowerCase()));
    const querySnapshot = await getDocs(q);
    
    if (!querySnapshot.empty) {
      const disableData = querySnapshot.docs[0].data();
      const disableEndTime = new Date(disableData.disableUntil.toDate());
      
      if (disableEndTime > new Date()) {
        // OTP is still disabled
        return true;
      } else {
        // OTP disable has expired, remove the record
        await updateDoc(doc(db, 'OTPDisable', querySnapshot.docs[0].id), {
          status: 'EXPIRED'
        });
      }
    }
...
const getAllUserPhoneNumbers = async () => {
  ...
  try {
    ...
    querySnapshot.docs.forEach(doc => {
      ...
      if (userData.contact) {
        phoneNumbers.push(userData.contact);
      }
    });
...
const sendOtpDisableNotification = async (userName, reason) => {
  try {
    const phoneNumbers = await getAllUserPhoneNumbers();
    const message = `SECURITY ALERT: User ${userName} has disabled their 2FA authentication. Reason: ${reason}. If this was not authorized, please contact admin immediately.`;
    
    const smsPromises = phoneNumbers.map(phoneNumber => sendSMS(phoneNumber, message));
    await Promise.all(smsPromises);
    
    return true;
  } catch (error) {
    console.error('Error sending OTP disable notifications:', error);
    return false;
  }
};
...
//       'LOGIN',
//       'SUCCESS',
//       'User successfully logged in without 2FA (disabled)'
//     );
...
const completeLoginWithoutOtp = async () => {
  try {
    // Ensure userDataForLogin exists before proceeding
    if (!userDataForLogin) {
      setError('User data not found. Please try logging in again.');
      return;
    }

    // Use the direct function with the state data
    await completeLoginWithoutOtpDirect(userDataForLogin, userDataForLogin.userDocId);
...
const handleOtpDisableFormChange = (field, value) => {
  setOtpDisableForm(prev => ({
    ...prev,
    [field]: value
  }));
};
  ...
  try {
    ...
    if (!querySnapshot.empty) {
      ...
      if (userData.password === password) {
        ...
        if (isOtpDisabled) {
          // Skip setting state and call the direct function immediately
          await completeLoginWithoutOtpDirect(userData, userDoc.id);
        } else {
...
//  const isOtpDisabled = await checkOtpDisableStatus(email);
//   if (isOtpDisabled) {
//   // Set user data before completing login
//   setUserDataForLogin({ ...userData, userDocId: userDoc.id });
  
//   // Wait a moment for state to update, then complete login
//   setTimeout(async () => {
//     await completeLoginWithoutOtp();
//   }, 100);
// } 
// else {
//   // Proceed with normal OTP flow  
...
const completeLoginWithoutOtpDirect = async (userData, userDocId) => {
  try {
    // Reset failed attempts
    await updateFailedAttempts(email, true);
